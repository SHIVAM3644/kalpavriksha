...
{   
    ...
    for (int i = 0; i < totalNumberOfStudents; i++)
    {
        ...
        while (!validInput)
        {
            ...
            int scannedValue = sscanf(line, "%hd %[^\n0123456789] %d %d %d",
            ...
            if (scannedValue != 5)
            {
                printf("Invalid input! Please enter in format: <rollNo> <name> <mark1> <mark2> <mark3>\n");
                continue;
            }
            ...
            for (int j = 0; j < i; j++)
            {
                if (studentInstance[j].rollNo == studentInstance[i].rollNo)
                {
                    isDuplicate = 1;
                    break;
                }
            ...
            if (isDuplicate)
            {
                printf("Error: Roll number %d already entered!\n", studentInstance[i].rollNo);
            }
            else
            {
                validInput = 1; 
